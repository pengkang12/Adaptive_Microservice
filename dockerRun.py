...
def main():
   ...
   # -------- Main testing loop Start ----------
    for line in open(args["file"]):
        ...
        vmstatCmdString = "python3.6 dockerstat_driver.py {} {} {} {} {}".format(exp_Nm,runtime,testDirPath,start_po,end_po)

        vmstatArgs = shlex.split(vmstatCmdString)
        vmstatResultFNm = testDirPath + "/dockerstatLog.txt"
        with open(vmstatResultFNm, 'w+') as vmstat_f:
            p2 = subprocess.Popen(vmstatArgs, stdout=vmstat_f, stderr=vmstat_f, shell=False)

        print("Started dockerStat: {0}".format(vmstatCmdString))
